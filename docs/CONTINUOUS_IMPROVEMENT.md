...
1. Writing UX: add a timeline view toggle that clusters entries by year and theme.
2. Contact conversion: add intent-specific follow-up prompts after request type selection.
3. SEO: add Organization and Person JSON-LD to profile and contact surfaces.
4. Performance: reduce opportunities page hydration cost with deferred non-critical controls.
5. Opportunities UX: add saved comparison presets (named trays) for recurring bid/no-bid workflows.
...
- Writing UX: ship a timeline toggle on `/writing/` that groups entries by year/theme for faster scanning.
