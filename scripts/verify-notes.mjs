...
const notesContentDir = path.join(repoRoot, 'src', 'content', 'writing', 'notes');
const generatedHtmlDir = path.join(repoRoot, 'src', 'generated', 'notes');
const publicPdfDir = path.join(repoRoot, 'public', 'notes');
...
function die(msg) {
	console.error(`verify-notes: ${msg}`);
	process.exitCode = 1;
}

if (!exists(notesContentDir)) {
	console.warn(`verify-notes: no notes content dir at ${notesContentDir}; skipping`);
	process.exit(0);
}

const mdFiles = fs
	.readdirSync(notesContentDir)
	.filter((f) => f.endsWith('.md') || f.endsWith('.mdx'))
	.sort();

if (mdFiles.length === 0) {
	console.warn('verify-notes: no note markdown files found; skipping');
	process.exit(0);
}

for (const file of mdFiles) {
	const mdPath = path.join(notesContentDir, file);
	...
	if (fm.type !== 'note') {
		die(`${slug}: type must be note (got ${JSON.stringify(fm.type)})`);
	}

	if (typeof fm.pdfPath !== 'string' || !fm.pdfPath.startsWith('/notes/')) {
		die(`${slug}: pdfPath must start with "/notes/" (got ${JSON.stringify(fm.pdfPath)})`);
	}
	...
	if (exists(manifestPath)) {
		...
		if (!exists(rawHtmlPath)) {
			console.warn(
				`verify-notes: ${slug}: raw HTML missing (${path.relative(repoRoot, rawHtmlPath)}); skipping HTML hash verification.`,
			);
		} else {
			const actualHtml = sha256File(rawHtmlPath);
			if (actualHtml !== expectedHtml) {
				die(
					`${slug}: HTML sha256 mismatch (expected ${expectedHtml}, got ${actualHtml}) comparing manifest to ${slug}.raw.html`,
				);
			}
		}
	}
...
if (!process.exitCode) {
	console.log(`verify-notes: OK (${mdFiles.length} note file(s) scanned)`);
}
