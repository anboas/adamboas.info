...
const checks = [
	...
	},
	{
		path: '/agents.json',
		type: 'json',
		requiredKeys: ['schema_version', 'resources', 'preferred_ingestion_order'],
		validate: validateAgents,
	},
	{
		path: '/agent-priority.json',
		type: 'json',
		requiredKeys: ['schema_version', 'ingestion_tiers', 'crawler_hints'],
		validate: validatePriority,
	},
	{
		path: '/changes.json',
		type: 'json',
		requiredKeys: ['release_version', 'event_log', 'discovery_endpoints'],
	...
	{
		path: '/writing/agent.json',
		type: 'json',
		requiredKeys: ['surface', 'primary_manifest', 'crawl_hints'],
		validate: validateWritingSurface,
	},
	{
		path: '/events/agent.json',
		type: 'json',
		requiredKeys: ['surface', 'related_routes', 'crawl_hints'],
		validate: validateEventsSurface,
	},
	{
		path: '/opportunities/agent.json',
		type: 'json',
		requiredKeys: ['surface', 'source_routes', 'crawl_hints'],
		validate: validateOpportunitiesSurface,
	},
	{
		path: '/opportunities/export.json',
		type: 'json',
		requiredKeys: ['schema_version', 'sources', 'exports'],
		validate: validateOpportunitiesExport,
	},
	{
		path: '/opportunities/export-sam.json',
		type: 'json',
		requiredKeys: ['schema_version', 'source', 'count', 'rows', 'variants'],
		validate: validateOpportunitiesExportSam,
	},
	{
		path: '/opportunities/export-sam-core.json',
		type: 'json',
		requiredKeys: ['schema_version', 'source', 'profile', 'rows'],
		validate: validateOpportunitiesExportSamCore,
	},
	{
		path: '/opportunities/export-sbir.json',
		type: 'json',
		requiredKeys: ['schema_version', 'source', 'mode', 'artifacts'],
		validate: validateOpportunitiesExportSbir,
	},
	{
		path: '/opportunities/freshness.json',
		type: 'json',
		requiredKeys: ['schema_version', 'sources'],
		validate: validateOpportunitiesFreshness,
	},
	{
		path: '/opportunities/lineage.json',
		type: 'json',
		requiredKeys: ['schema_version', 'lineage'],
		validate: validateOpportunitiesLineage,
	},
	{
		path: '/integrity.json',
		type: 'json',
	...
	{ path: '/for-agents/', type: 'html', mustContain: ['For Agents', 'Discovery Endpoints'] },
	{
		...
		type: 'json',
		requiredKeys: ['canonical_manifest', 'discovery'],
		validate: validateWellKnownManifest,
	},
	{ path: '/.well-known/llms.txt', type: 'text', mustContain: ['/llms.txt'] },
	{
		path: '/schemas/agents.schema.json',
		type: 'json',
		requiredKeys: ['$schema', '$id', 'type'],
		validate: validateSchemaDoc,
	},
	{
		path: '/schemas/agent-priority.schema.json',
		type: 'json',
		requiredKeys: ['$schema', '$id', 'type'],
		validate: validateSchemaDoc,
	},
	{
		path: '/schemas/changes.schema.json',
		type: 'json',
		requiredKeys: ['$schema', '$id', 'type'],
		validate: validateSchemaDoc,
	},
	{
		path: '/schemas/opportunities-export.schema.json',
		type: 'json',
		requiredKeys: ['$schema', '$id', 'type'],
		validate: validateSchemaDoc,
	},
	{
		path: '/schemas/opportunities-export-sam.schema.json',
		type: 'json',
		requiredKeys: ['$schema', '$id', 'type'],
		validate: validateSchemaDoc,
	},
	{
		path: '/schemas/opportunities-export-sbir.schema.json',
		type: 'json',
		requiredKeys: ['$schema', '$id', 'type'],
		validate: validateSchemaDoc,
	},
	{
		path: '/schemas/opportunities-freshness.schema.json',
		type: 'json',
		requiredKeys: ['$schema', '$id', 'type'],
		validate: validateSchemaDoc,
	},
	{
		path: '/schemas/opportunities-lineage.schema.json',
		type: 'json',
		requiredKeys: ['$schema', '$id', 'type'],
		validate: validateSchemaDoc,
	},
...
function assertHasEndpointWithPath(urls, path, label) {
	...
	assert(
		urls.some((url) => typeof url === 'string' && url.includes(path)),
		`${label} missing endpoint containing: ${path}`,
	);
}
...
function validateAgents(data) {
	...
	for (const resource of data.resources) {
		...
		assert(
			...
			`/agents.json resource type missing: ${resource.id}`,
		);
...
function validatePriority(data) {
	assert(
		Array.isArray(data.ingestion_tiers) && data.ingestion_tiers.length >= 3,
		'/agent-priority.json ingestion_tiers invalid',
	);
	let lastTier = 0;
	for (const tier of data.ingestion_tiers) {
		...
		lastTier = tier.tier;
		assert(
			Array.isArray(tier.endpoints) && tier.endpoints.length > 0,
			`/agent-priority tier ${tier.tier} endpoints missing`,
		);
		for (const endpoint of tier.endpoints) assertAbsUrl(endpoint, `/agent-priority tier ${tier.tier} endpoint`);
		assert(
			typeof tier.refresh_hint === 'string' && tier.refresh_hint.length > 0,
			`/agent-priority tier ${tier.tier} refresh_hint missing`,
		);
	}
	assert(
...
function validateChanges(data) {
	...
	assert(
		typeof data.event_log?.count === 'number' && data.event_log.count >= 1,
		'/changes.json event_log.count invalid',
	);
	assert(
		typeof data.event_log?.latest_event_id === 'string' && data.event_log.latest_event_id.length > 0,
		'/changes.json latest_event_id missing',
	);
	assert(
		typeof data.event_log?.latest_event_ts === 'string' && data.event_log.latest_event_ts.length > 0,
		'/changes.json latest_event_ts missing',
	);
...
function validateOpportunitiesSurface(data) {
	...
	assert(
		Array.isArray(data.query_hints?.source_param),
		'/opportunities/agent.json query_hints.source_param must be array',
	);
...
function validateOpportunitiesExport(data) {
	assert(data.schema_version === '1.3', '/opportunities/export.json schema_version must be 1.3');
	...
	assert(
		typeof data.sources?.sam?.count === 'number' && data.sources.sam.count >= 1,
		'/opportunities/export.json sam count invalid',
	);
	assert(
		typeof data.sources?.sam?.snapshot_id === 'string' && /^[a-f0-9]{64}$/.test(data.sources.sam.snapshot_id),
		'/opportunities/export.json sam snapshot_id invalid',
	);
	...
	assert(
		typeof data.sources?.sbir?.mode === 'string' && data.sources.sbir.mode.length > 0,
		'/opportunities/export.json sbir mode missing',
	);
...
function validateOpportunitiesExportSam(data) {
	assert(data.schema_version === '1.3', '/opportunities/export-sam.json schema_version must be 1.3');
	...
	assert(
		typeof data.returned_count === 'number' && data.returned_count >= 1,
		'/opportunities/export-sam.json returned_count invalid',
	);
	...
	assert(
		typeof data.lineage?.source_dataset_path === 'string',
		'/opportunities/export-sam.json lineage.source_dataset_path missing',
	);
	assert(
		typeof data.lineage?.snapshot_id === 'string' && /^[a-f0-9]{64}$/.test(data.lineage.snapshot_id),
		'/opportunities/export-sam.json lineage.snapshot_id invalid',
	);
}

function validateOpportunitiesExportSamCore(data) {
	assert(data.schema_version === '1.1', '/opportunities/export-sam-core.json schema_version must be 1.1');
	...
	assert(
		typeof data.lineage?.snapshot_id === 'string' && /^[a-f0-9]{64}$/.test(data.lineage.snapshot_id),
		'/opportunities/export-sam-core.json lineage.snapshot_id invalid',
	);
	if (data.rows.length > 0) {
		const row = data.rows[0];
		assert(
			Object.hasOwn(row, 'id') && Object.hasOwn(row, 'title'),
			'/opportunities/export-sam-core.json row missing required fields',
		);
		assert(
			!Object.hasOwn(row, 'solicitation_number'),
			'/opportunities/export-sam-core.json row should omit solicitation_number',
		);
	}
}

function validateOpportunitiesFreshness(data) {
	assert(data.schema_version === '1.1', '/opportunities/freshness.json schema_version must be 1.1');
	assert(typeof data.sources?.sam?.count === 'number', '/opportunities/freshness.json sources.sam.count missing');
	assert(
		typeof data.sources?.sam?.snapshot_id === 'string' && /^[a-f0-9]{64}$/.test(data.sources.sam.snapshot_id),
		'/opportunities/freshness.json sam snapshot_id invalid',
	);
...
function validateOpportunitiesLineage(data) {
	...
	assert(
		typeof data.lineage?.sam?.full_snapshot_id === 'string' && /^[a-f0-9]{64}$/.test(data.lineage.sam.full_snapshot_id),
		'/opportunities/lineage.json sam full_snapshot_id invalid',
	);
	assert(
		typeof data.lineage?.sam?.core_snapshot_id === 'string' && /^[a-f0-9]{64}$/.test(data.lineage.sam.core_snapshot_id),
		'/opportunities/lineage.json sam core_snapshot_id invalid',
	);
...
function validateOpportunitiesExportSbir(data) {
	...
	assert(
		typeof data.lineage?.artifact_repo === 'string' && data.lineage.artifact_repo.length > 0,
		'/opportunities/export-sbir.json lineage.artifact_repo missing',
	);
	assert(
		typeof data.lineage?.artifact_path === 'string' && data.lineage.artifact_path.length > 0,
		'/opportunities/export-sbir.json lineage.artifact_path missing',
	);
}
...
function validateWellKnownManifest(data) {
	...
	assert(
		data.discovery.surface_maps.length >= 3,
		'/.well-known/agent-manifest.json discovery.surface_maps expected >= 3',
	);
	for (const mapUrl of data.discovery.surface_maps)
		assertAbsUrl(mapUrl, '/.well-known/agent-manifest.json discovery.surface_maps entry');
	assert(Array.isArray(data.discovery?.exports), '/.well-known/agent-manifest.json discovery.exports must be array');
	for (const exportUrl of data.discovery.exports)
		assertAbsUrl(exportUrl, '/.well-known/agent-manifest.json discovery.exports entry');
	assertHasEndpointWithPath(
		data.discovery.exports,
		'/opportunities/export.json',
		'/.well-known/agent-manifest.json discovery.exports',
	);
	assertHasEndpointWithPath(
		data.discovery.exports,
		'/opportunities/export-sam.json',
	...
	for (const metadataUrl of data.discovery.metadata)
		assertAbsUrl(metadataUrl, '/.well-known/agent-manifest.json discovery.metadata entry');
	assertHasEndpointWithPath(
		data.discovery.metadata,
		'/opportunities/freshness.json',
		'/.well-known/agent-manifest.json discovery.metadata',
	);
	assertHasEndpointWithPath(
		data.discovery.metadata,
		'/opportunities/lineage.json',
	...
	for (const schemaUrl of data.discovery.schemas)
		assertAbsUrl(schemaUrl, '/.well-known/agent-manifest.json discovery.schemas entry');
	assertHasEndpointWithPath(
		data.discovery.schemas,
		'/schemas/agents.schema.json',
		'/.well-known/agent-manifest.json discovery.schemas',
	);
	assertHasEndpointWithPath(
		data.discovery.schemas,
...
function validateSchemaDoc(data) {
	assert(
		typeof data.$schema === 'string' && data.$schema.includes('json-schema.org'),
		'schema doc missing valid $schema',
	);
...
function validateIntegrity(data) {
	...
	for (const item of data.tracked_source_integrity) {
		...
		assert(
			...
			`/integrity.json invalid sha256 for ${item.path}`,
		);
